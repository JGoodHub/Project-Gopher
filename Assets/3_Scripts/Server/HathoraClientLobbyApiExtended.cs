...
namespace Hathora.Core.Scripts.Runtime.Client.ApiWrapper
{
    ...
    /// </summary>
    public class HathoraClientLobbyApiExtended : HathoraClientApiWrapperBase
    {
        ...
        public async Task<Lobby> ClientSetLobbyStateAsync(
            string roomId,
            SetLobbyStateRequest setLobbyStateRequest,
            CancellationToken _cancelToken = default)
        {
            Debug.Log("[NetHathoraClientLobbyApi.ClientSetLobbyStateAsync] " +
                      $"<color=yellow>roomId: {roomId} setLobbyStateRequest: {setLobbyStateRequest}</color>");
            
            Lobby lobby;
            try
            {
                lobby = await lobbyApi.SetLobbyStateAsync(
                    HathoraClientConfig.AppId,
                    roomId,
            ...
            {
                HandleApiException(
                    nameof(HathoraClientLobbyApiExtended),
                    nameof(ClientSetLobbyStateAsync), 
                    apiException);
                
                return null; // fail
            }

            Debug.Log($"[NetHathoraClientAuthApi.ClientSetLobbyStateAsync] Success: " +
                      $"<color=yellow>lobby: {lobby.ToJson()}</color>");            
